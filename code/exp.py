...
def cnn_lstm_UA_DA(args):
    ...
    if args.model_to_run == 'cnn_lstm_UA_DA':
        cnn_model = CNN_LSTM_UA_DA_Model('CNN_LSTM_UA_DA_Model', num_labels, vocab_size, args.embedding_size, max_d, max_q, max_s,
    ...
    if args.model_to_run == 'cnn_lstm_UA':
        cnn_model = CNN_LSTM_UA_Model('CNN_LSTM_UA_Model', num_labels, vocab_size, args.embedding_size, max_d, max_q, max_s,
            nb_filters_utterance=args.utterance_filters, nb_filters_query=args.query_filters, learning_rate=args.learning_rate,
            dropout=args.dropout, nb_hidden_unit=args.hidden_size)
    ...
    all_dev = gen_examples(dev_x1, dev_x2, dev_matchscore, dev_l, dev_y, dev_qmask, dev_dmask, args.batch_size)
    ...
    for epoch in range(args.nb_epoch):
        ...
        for idx, (mb_x1, mb_x2, mb_match, mb_l, mb_y, mb_qmask, mb_dmask) in enumerate(all_train):
            ...
            hist = cnn_model.fit(newx1+new_match+[np.array(mb_x2)]+[np.array(mb_l)]
